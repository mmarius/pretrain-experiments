...
class InsertionMapReader:
    ...
    def __init__(self, hdf5_path: str, cache_size_mb: int = 64):
        ...
        logger.debug(f"InsertionMapReader initialized with {len(self._keys_array)} indices")
...
class InsertionMapWriter:
    ...
    def write_dict(self, insertion_map: Dict[int, List[Tuple[int, List[int]]]],
        ...
        """
        logger.debug(f"Writing {len(insertion_map)} entries to {self.hdf5_path} (mode: {mode})")
        ...
        logger.debug(f"Successfully wrote data to {self.hdf5_path}")
    ...
    def append_dict(self, insertion_map: Dict[int, List[Tuple[int, List[int]]]]) -> None:
        ...
        """
        logger.debug(f"Appending {len(insertion_map)} entries to {self.hdf5_path}")
        ...
        logger.debug(f"Successfully appended data to {self.hdf5_path}")
    ...
    def save_optimized(self, output_path: str) -> None:
        ...
        """
        logger.debug(f"Creating optimized file {output_path} from {self.hdf5_path}")
        ...
        logger.debug(f"Successfully created optimized file with {len(keys)} indices, "
    ...
    def read_dict(self) -> Dict[int, List[Tuple[int, List[int]]]]:
        ...
        """
        logger.debug(f"Reading entire insertion map from {self.hdf5_path}")
        result = {}
        ...
        logger.debug(f"Successfully read {len(result)} entries")
        return result
