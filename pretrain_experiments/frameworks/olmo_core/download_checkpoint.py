...
def discover_directory_files(url, dir_name):
    ...
    if dir_name == "model_and_optim":
        ...
        logger.info("  Discovering ranks (probing shard 0)...")
        ranks_with_data = []
        ...
        if not ranks_with_data:
            logger.warning("  No non-empty rank files found for shard 0")
            return files
        
        logger.info(f"  Found {len(ranks_with_data)} ranks with data: {ranks_with_data}")
        ...
        logger.info(f"  Discovering shards (probing rank {probe_rank})...")
        shards_with_data = []
        ...
        logger.info(f"  Found {len(shards_with_data)} shards with data")
        ...
        logger.info(f"  Total: {len(files)} checkpoint files to download")
...
def download_checkpoint(url, save_dir):
   ...
   logger.info(f"Saving to: {base_path}")
   ...
   for dir_name in available_dirs:
       ...
       logger.info(f"Will download {len(dir_files)} files from {dir_name}/")
   ...
   if skipped_files:
       logger.info(f"Skipped (empty): {len(skipped_files)} files")
   if failed_files:
...
def main():
    ...
    if args.command == 'list':
        ...
        if not urls:
            logger.error(f"Step {args.step} not found in the CSV file.")
            logger.info("Use list argument to see available checkpoint steps.")
            return
